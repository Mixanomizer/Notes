...
createBtn.onclick = function () {
  if (inputElement.value.length === 0) {
    inputElement.style.border="3px solid #ff0000"
    setTimeout(() => {
      inputElement.style.border="1px solid #c6d9ff"
    }, 4000);
    return;
  }
  const newNote = {
...
function getNoteTemplate(note, index) {
  return `<li class="pading">
          <span class="${
            note.completed ? "text-decoration-line-trought" : ""
          }">${note.title}</span>
          <span class="btn-position">
            <span  class="btn-status btn-${
              note.completed ? "completed" : "success"
            } btn" data-index=${index} data-type="completed">&check;</span>
